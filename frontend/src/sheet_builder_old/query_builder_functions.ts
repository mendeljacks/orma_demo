...
export const query_path_to_entity_name = (path_array: any, query: any) => {
  ...
  for (const i of range(0, path_array.length)) {
    ...
    if (path_el === "$any_path") {
      ...
      entity_name = last(any_route.filter((el: any) => !!el))
      continue
    }

    const path_el_is_table_name = is_entity_name(
      path_el,
...
export const switch_clause = action(
  (path_to_clause: any, new_clause: string, query: any) => {
    const old_clause = last(path_to_clause)
    const path_to_clause_object = dropLast(1, path_to_clause)
    const old_clause_object = safe_path_or(
      undefined,
      path_to_clause_object,
      query
    )
    ...
    const new_clause_object = clause_mapping(
      old_clause_object,
      new_clause,
      old_clause
    )
...
export const get_nested_path_edge_tables = (
  path_to_nested_path: any[],
  query: any
) => {
  const nested_path = safe_path_or(
    undefined,
    path_to_nested_path,
    query
  ).filter((el: string) => !!el)
...
export const get_possible_edge_entity_names = (
  ...
  const entity_names = edges
    ...
    .filter(
      (entity_name) => !existing_subquery_entity_names.includes(entity_name)
    )

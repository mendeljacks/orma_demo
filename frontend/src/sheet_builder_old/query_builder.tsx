    ...
    return (
      <>
        <QueryObject>
          ...
          <QueryGroupBy path_array={path_array} query={query} />
          <QueryOrderBy path_array={path_array} query={query} />
          <QuerySubqueries path_array={path_array} query={query} />
...
const QuerySelect = observer(
  ({ path_array, query }: { path_array: any; query: any }) => {
    ...
    const selects = get_select(
      subquery,
      path_array,
      store.introspect.schema
    ).filter((el) => typeof el === "string") as string[]

    const possible_fields = get_field_names(
      last(path_array),
      store.introspect.schema
    )
...
const QueryPagination = observer(
  ({ path_array, query }: { path_array: any; query: any }) => {
  ...
  ({ path_array, query }: { path_array: any; query: any }) => {
    return (
      <Grid container alignItems="center">
        ...
        <Grid item>
          <QueryClauseDeleteButton
            path_array={dropLast(1, path_array)}
            query={query}
          />
        </Grid>
      </Grid>
  ...
  ({ path_array, query }: { path_array: any; query: any }) => {
    ...
    return (
      <TextField
        ...
        onChange={action((e) =>
          assoc_path_mutate(path_array, e.target.value, query)
        )}
        variant="outlined"
        size="small"
      >
        {get_field_names(entity_name, store.introspect.schema).map(
          (field_name) => (
  ...
  ({ path_array, query }: { path_array: any; query: any }) => {
    return (
      <TextField
        ...
        variant="outlined"
        helperText={
          safe_path_or(undefined, path_array, query) === null ? "" : undefined
        }
        size="small"
      />
    )
...
const QueryJoinedClauseChooser = observer(
  ({ path_array, query }: { path_array: any; query: any }) => {
    const operation_name = last(path_array)
    const subquery_count =
      safe_path_or(undefined, [...path_array, 1], query)?.length || 0
    ...
    return (
      ...
      >
        <MenuItem value={"none"} disabled={subquery_count > 1}>
          <em>None</em>{" "}
          {subquery_count > 1 && <>&nbsp;(Delete terms to remove)</>}
        </MenuItem>
        <MenuItem value={"$and"}>And</MenuItem>
        <MenuItem value={"$or"}>Or</MenuItem>
        <MenuItem value={"$any_path"}>Any</MenuItem>
  ...
  ({ path_array, query }: { path_array: any; query: any }) => {
    return (
      <IconButton
        style={{ border: "2px dashed grey", borderRadius: "0px" }}
        onClick={action((e: any) =>
          assoc_append(path_array, query, default_where_clause)
        )}
        size="large"
      >
        <MdAdd />

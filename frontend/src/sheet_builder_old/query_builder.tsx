...
const BatchLineInput = observer(
  ({ path_array, query }: { path_array: any; query: any }) => {
    return (
      <TextField
        value={safe_path_or([], path_array, query).map(el => el?.$escape).join("\n")}
        onChange={action((e) => {
  ...
  ({ path_array, query }: { path_array: any; query: any }) => {
    ...
    return (
      <TextField
        select
        value={
          is_field_name(entity_name, field_name, store.schema) ? field_name : ""
        }
        onChange={action((e) =>
        ...
        size="small"
      >
        {get_field_names(entity_name, store.schema).map((field_name) => (

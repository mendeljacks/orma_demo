...
export const byo_query_fn = async (sqls: { sql_string }[], connection, database_type: DbType) => {
...
export const mutate_handler = (
    ...
    return trans(async connection => {
        apply_inherit_operations_macro(mutation, undefined)
        ...
        const mutation_results = await orma_mutate(
            mutation,
            sqls => byo_query_fn(sqls, connection, database_type),
...
export const query_handler = (query: any, connection: any, database_type: DbType) => {
    return orma_query(query, orma_schema as unknown as OrmaSchema, sqls =>
        byo_query_fn(sqls, connection, database_type)
    )
}

export const introspect_handler = (db: string, connection: any, database_type: DbType) => {
    return orma_introspect(db, sqls => byo_query_fn(sqls, connection, database_type), {
        database_type
    })
}

export const get_pool_trans = async (req_query: {
    database_type: DbType
    mysql?: any
    pg?: any
}) => {
    const database_type = req_query.database_type
    const mysql = JSON.parse(req_query.mysql || '{}')
    const pg = JSON.parse(req_query.pg || '{}')

    if (database_type === 'mysql') {
        const pool = await get_pool_mysql(mysql)
        return { pool, trans: mysql_trans(pool), db: mysql.database, database_type }
    }
    if (database_type === 'postgres') {
        const pool = get_pool_pg(pg.connection_string || process.env.pg)
        return { pool, trans: pg_trans(pool), db: pg.database, database_type }
    }
}

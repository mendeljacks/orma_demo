...
const QuerySimpleClause = observer(({ path_array, query }: { path_array: any; query: any }) => {
    return (
        <Grid container alignItems='center'>
            ...
            <Grid item>
                <QueryClauseValueEditor path_array={[...path_array, 1]} query={query} />
            </Grid>
            <Grid item>
...
const BatchLineInput = observer(({ path_array, query }: { path_array: any; query: any }) => {
    return (
        <TextField
            value={safe_path_or([], path_array, query).join('\n')}
            onChange={action(e => {
...
const QueryClauseFieldSelect = observer(
    ({ path_array, query }: { path_array: any; query: any }) => {
        ...
        return (
            <TextField
                select
                value={is_field_name(entity_name, field_name, store.schema) ? field_name : ''}
